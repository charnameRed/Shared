...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, groups=1, base_width=64, norm_layer=None,
    ):
...
class ResNet(nn.Module):
    def __init__(
        self, block, layers, zero_init_residual=False, groups=1, width_per_group=64, norm_layer=None,
    ):
    ...
    def _make_layer(self, block, planes, blocks, stride=(1, 1)):
        ...
        downsample = None
        if any(s > 1 for s in stride) or self.inplanes != planes * block.expansion:
...
class ResNet18Gray(nn.Module):
    def __init__(self):
        ...
        norm_layer = nn.BatchNorm2d
        self.model = ResNet(BasicBlock, [2, 2, 2, 2], norm_layer=norm_layer)
